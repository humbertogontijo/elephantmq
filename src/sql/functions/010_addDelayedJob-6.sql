-- Mirrors ref/bullmq/src/commands/addDelayedJob-6.lua

create or replace function :EMQ_SCHEMA.emq_add_delayed_job_v1(
  p_queue_id bigint,
...
begin
  ...
  if nullif(trim(p_dedup_id), '') is not null
    ...
    if v_dedup_existing is not null
      ...
      if (v_de_opts->>'keepLastIfActive') = 'true'
        ...
        -- BullMQ `deduplicateJob.lua`: when a dedup hit fires with
        -- `extend=true` and a positive TTL, push the expiration forward so
        -- subsequent re-adds within the new window continue to dedup.
  ...
  -- replace: remove prior delayed job for this dedup id so the new delayed job can be inserted
  -- (BullMQ deduplicateJob.lua + removeDelayedJob). If the holder is not delayed (active/completed),
  -- return that job id and do not insert (ZREM miss path).
  if nullif(trim(p_dedup_id), '') is not null
    ...
    if v_dedup_existing is not null then
      ...
      where j.queue_id = p_queue_id
        and j.job_id = v_dedup_existing
        and j.state = 'delayed':: :EMQ_SCHEMA.emq_job_state;
  ...
  values (
    ...
    'delayed':: :EMQ_SCHEMA.emq_job_state,
    v_prio, v_process_at, v_delay, to_timestamp(p_timestamp_ms / 1000.0), v_max_attempts,
  ...
  perform :EMQ_SCHEMA.emq_emit_event_v1(
    p_queue_id,
    'delayed',
    jsonb_build_object('jobId', v_job_id, 'delay', p_timestamp_ms + v_delay)
  );
