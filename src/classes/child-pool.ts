...
export class ChildPool {
  ...
  async retain(processFile: string): Promise<Child> {
    ...
    if (child) {
      this.retained[this.childPid(child)] = child;
      return child;
    }
  ...
  release(child: Child): void {
    delete this.retained[this.childPid(child)];
  ...
  private childPid(child: Child): number {
    const pid = child.pid;
    if (typeof pid !== 'number' || Number.isNaN(pid)) {
      throw new Error('child has no usable pid/threadId yet');
    }
    return pid;
  }
}
