...
/** Detect a `pg.Pool` instance by duck-typing. */
export function isPgPool(obj: any): obj is Pool {
  if (!obj) {
    return false;
  }
  return typeof obj.query === 'function' && typeof obj.connect === 'function';
}
...
export async function removeAllQueueData(
  client: EmqClient,
  queueName: string,
  prefix = process.env.ELEPHANTMQ_PREFIX || 'emq',
  schema = process.env.ELEPHANTMQ_SCHEMA || 'public',
): Promise<void | boolean> {
  const esc = schema.replace(/"/g, '""');
  await client.query(
    `delete from "${esc}".emq_queues where prefix = $1 and name = $2`,
    [prefix, queueName],
  );
}
